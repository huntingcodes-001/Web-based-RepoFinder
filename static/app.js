...
function displayResults(repos) {
    ...
    repos.forEach(repo => {
        ...
        repoDiv.innerHTML = `
            ...
            <p><i class="fas fa-link"></i> <strong>URL:</strong> <a href="${repo.html_url}" target="_blank">${repo.html_url}</a></p>
            <button id="${cloneButtonId}" class="button is-small is-link" onclick="cloneRepo('${repo.clone_url}', '${repo.name}', '${cloneButtonId}')">Clone</button>
